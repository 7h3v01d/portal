...
def test_happy_path_authenticates_peer_key():
    ...
    client_msg = build_auth(client, ROLE_CLIENT, cb)
    server_msg = build_auth(server, ROLE_SERVER, cb)

    # Server verifies the client; client verifies the server.
    peer_at_server = verify_auth(client_msg, ROLE_CLIENT, cb)
    peer_at_client = verify_auth(server_msg, ROLE_SERVER, cb)
...
def test_relay_across_different_channels_fails():
    ...
    client_msg = build_auth(client, ROLE_CLIENT, cb_leg1)  # made on leg 1
    with pytest.raises(IdentityError):
        verify_auth(client_msg, ROLE_CLIENT, cb_leg2)  # replayed onto leg 2
...
def test_reflection_back_to_sender_fails():
    ...
    client_msg = build_auth(client, ROLE_CLIENT, cb)
    with pytest.raises(IdentityError):
        verify_auth(client_msg, ROLE_SERVER, cb)  # expected a server-role message


def test_tampered_signature_fails():
    cb = secrets.token_bytes(48)
    client = Ed25519Identity.generate("Leon")
    msg = build_auth(client, ROLE_CLIENT, cb)
    sig = bytearray.fromhex(msg["signature"])
    sig[0] ^= 0x01
    msg["signature"] = bytes(sig).hex()
    with pytest.raises(IdentityError):
        verify_auth(msg, ROLE_CLIENT, cb)
...
def test_malformed_messages_rejected(bad):
    with pytest.raises(IdentityError):
        verify_auth(bad, ROLE_CLIENT, secrets.token_bytes(48))
